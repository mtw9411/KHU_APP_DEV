...
public class Fragement_navi extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final BottomNavigationView bottomNavigationview = (BottomNavigationView) findViewById(R.id.bottomNavigationView_navi);
    ...
    public void setFrag(int n){    //프래그먼트를 교체하는 작업을 하는 메소드를 만들었습니다
        ...
        switch (n){
            case 0:
                fragmentTransaction.replace(R.id.Frame_navi, main_fragment);  //replace의 매개변수는 (프래그먼트를 담을 영역 id, 프래그먼트 객체) 입니다.
                fragmentTransaction.commit();
                break;
            case 1:
                fragmentTransaction.replace(R.id.Frame_navi, timeline_fragment);  //replace의 매개변수는 (프래그먼트를 담을 영역 id, 프래그먼트 객체) 입니다.
                fragmentTransaction.commit();
                break;
            case 2:
                fragmentTransaction.replace(R.id.Frame_navi, alarm_fragment);  //replace의 매개변수는 (프래그먼트를 담을 영역 id, 프래그먼트 객체) 입니다.
                fragmentTransaction.commit();
                break;
            case 3:
                fragmentTransaction.replace(R.id.Frame_navi,mypage_fragment);
                fragmentTransaction.commit();
                break;
        }
    }
